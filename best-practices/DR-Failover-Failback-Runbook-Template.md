# <CUSTOMER> Snowflake DR Failover / Failback Runbook (Template)
### Business Critical Edition · <CLOUD> (<PROD_ACCOUNT>, <PRIMARY_REGION>) ⇄ <DR_REGION> (<DR_ACCOUNT>)
### Two Modes: Mode A — Real DR Drill (destructive) · Mode B — Non-Destructive Test (clone-and-test, recommended first)

> **How to use.** Copy into the customer's folder, rename to `<CUSTOMER>-DR-Failover-Failback-Runbook.md`, and replace every `<TOKEN>` per [`PLACEHOLDERS.md`](./PLACEHOLDERS.md). Remove the private-connectivity steps (§1.4 notes, §5.1.7, §6.8, §8/§11 lines) if the customer uses public connectivity. Section numbers are referenced by the summary and best-practices — keep them stable.
>
> **Scope.** DR for the `<WORKLOAD>` workload (`<APP_DB>`) only. Other applications on `<PROD_ACCOUNT>` have **no DR plan** and **must not be disrupted** by a `<WORKLOAD>` failover. This is the central design constraint.
>
> **Companion documents.** `<CUSTOMER>-DR-Summary.md` · `<CUSTOMER>-DR-Best-Practices.md`

---

## 0. Why this document exists

`<WORKLOAD>` is a tenant on a **shared** `<PROD_ACCOUNT>` account. It needs regional DR, but the other databases on the account do not. The runbook lets `<WORKLOAD>` fail over to `<DR_REGION>` **independently**, while leaving every non-`<WORKLOAD>` workload read-write on production.

Independent failover groups make that possible:

| Group | Members | Promoted during a `<WORKLOAD>` failover? |
|---|---|---|
| `FG_ACCOUNT_LEVEL` | Users, roles, warehouses, resource monitors, network policies, integrations | **Yes** |
| `FG_<WORKLOAD>_DB` | `<APP_DB>` + dependent objects | **Yes** |
| `FG_NON_<WORKLOAD>_DBS` | All other in-scope production databases | **No** (stays on production) |

Two wrinkles to confirm per customer:
1. **Client Redirect via a named Connection** (`<WORKLOAD>_CONN` → `<CONNECTION_URL>`) gives consumers a stable URL — **but only if every writer is migrated to it first.**
2. **`<PRIVATE_CONNECTIVITY>` breaks automatic redirect.** Every failover needs a **manual DNS change** (connection URL **and** OCSP URL), the dominant RTO component. *(Omit if public connectivity.)*

---

## 1. `<WORKLOAD>` Environment Reference

### 1.1 How failover scoping works (why other workloads keep running)

Common question: *"When `<WORKLOAD>` fails over and DR becomes primary, won't the rest of the production account go read-only?"* **No.**
...
**Post-failover state on production** (after promoting `FG_ACCOUNT_LEVEL` + `FG_<WORKLOAD>_DB` on DR):

| Object on production | Member of a promoted group? | Role after promotion | Read/Write on production? |
|---|---|---|---|
| `<APP_DB>` | Yes (`FG_<WORKLOAD>_DB`) | becomes **secondary** | Read-only |
| Databases in `FG_NON_<WORKLOAD>_DBS` | No (group not promoted) | stays **primary** | **Read/Write** |
...
| Cloud | **`<CLOUD>`** | — |
| Primary region | `<PRIMARY_REGION>` | Production account |
| DR region | `<DR_REGION>` | Must differ from primary |
| Org | `<ORG>` | Same org for both accounts |
| Account locator — Primary | `<PROD_ACCOUNT>` | — |
| Account locator — DR | `<DR_ACCOUNT>` | `<DR_REGION>` |
| Workload database | `<APP_DB>` | — |
| Connection object | `<WORKLOAD>_CONN` *(confirm)* | Client Redirect |
| Connection URL | `<CONNECTION_URL>` | `<WORKLOAD>` consumers only |
| Private endpoint — Primary | `<PRIMARY_PE>` | *(if private connectivity)* |
| Private endpoint — DR | `<DR_PE>` | *(if private connectivity)* |
| DNS TTL (connection + OCSP) | **60–300 seconds** | Set in advance |
| Failover group refresh interval | `<REFRESH_INTERVAL>` | Use SQL, not GUI |

> **Action.** Replace every `<TOKEN>`; append the database list for `FG_NON_<WORKLOAD>_DBS`, the role/warehouse list for `FG_ACCOUNT_LEVEL`, and the dependent-object inventory for `FG_<WORKLOAD>_DB` before the first drill.
...
| Storage integrations | ✅ (object) | **DR service identity needs `<STORAGE_RBAC_ROLE>`** on the same `<OBJECT_STORE>` |
...
| **Refresh schedule** | `SHOW REPLICATION GROUPS` | **RPO (definition)** | The cadence on `FG_<WORKLOAD>_DB` *is* the worst-case RPO. |

#### 1.4.1 Per-table churn — the primary RPO/RTO driver

```sql
-- ═══════════════════════════════════════════════════════════════════════════
-- §1.4.1 Hottest tables in <APP_DB>.
...
**How it feeds the analysis.** The few highest-churn tables effectively dictate the achievable RPO (how often they can be replicated) and are the long pole in RTO (their Dynamic Table consumers reinitialize from them). Low-churn reference tables are irrelevant to both; large tables with **zero** fail-safe are static and cheap to protect.

#### 1.4.2 Database-wide churn ratio — the RTO indicator

```sql
-- ═══════════════════════════════════════════════════════════════════════════
-- §1.4.2 Workload volatility for <APP_DB> as a whole.
...
-- ═══════════════════════════════════════════════════════════════════════════
SHOW REPLICATION GROUPS;   -- inspect REPLICATION_SCHEDULE on FG_<WORKLOAD>_DB
...
**How it feeds the analysis.** If the business wants a tighter RPO than the §1.4.1 hot tables make affordable, this is the evidence for the trade-off: accept the cost of more frequent refreshes, or reduce churn (split a hot table, lower time-travel retention).

#### 1.4.4 Turning the analytics into DR tiers

| Tier | Defined by the analytics as | RPO/RTO treatment |
|---|---|---|
| **Tier 1 — Critical** | Highest churn **and** feeds Dynamic Tables / semantic views / shares | Drives the RPO target; row-count + checksum parity verified **first** after every refresh and post-failover |
...
| **Tier 3 — Standard** | Cold / low churn / static | Replicates with the group; spot-check only |
...
| Is `<APP_DB>` on production made read-only? | **Yes** (briefly) | **No** |
...
| Confirm `<WORKLOAD>_CONN` Connection object exists; finalize name | TBD | ☐ |
| **Migrate all `<APP_DB>` writers/readers to the connection URL** | TBD | ☐ |
| Complete object audit (§1.3); resolve append-only streams + dangling references | TBD | ☐ |
| Grant DR service identity `<STORAGE_RBAC_ROLE>` on shared `<OBJECT_STORE>` | TBD | ☐ |
| Build DR-side `<PIPE_EVENT_PLUMBING>` for pipes | TBD | ☐ |
| Resolve every inbound data share (auto-fulfillment or dual share) | TBD | ☐ |
| *(Private connectivity)* Pre-create endpoints; pre-stage DNS (connection + OCSP); set TTL 60–300s | Network team | ☐ |
...
```sql
-- ═══════════════════════════════════════════════════════════════════════════
-- §3.4 PRE-FLIGHT — <PROD_ACCOUNT> (<PRIMARY_REGION>)
-- ═══════════════════════════════════════════════════════════════════════════
...
FROM TABLE(INFORMATION_SCHEMA.REPLICATION_GROUP_REFRESH_HISTORY('FG_<WORKLOAD>_DB'))
ORDER BY last_refresh_end_time DESC LIMIT 5;

-- 3. Member databases of each group
SHOW DATABASES IN FAILOVER GROUP FG_<WORKLOAD>_DB;
SHOW DATABASES IN FAILOVER GROUP FG_NON_<WORKLOAD>_DBS;
...
SELECT * FROM TABLE(INFORMATION_SCHEMA.REPLICATION_GROUP_DANGLING_REFERENCES('FG_<WORKLOAD>_DB'));

-- 5. Connection object + its primary side
SHOW CONNECTIONS;

-- 6. Non-replicable / blocking object classes in <APP_DB>
SHOW STREAMS IN DATABASE <APP_DB>;          -- inspect MODE for APPEND_ONLY (must be zero)
SHOW EXTERNAL TABLES IN DATABASE <APP_DB>;
SELECT 'HYBRID TABLES' AS object_class, COUNT(*) AS cnt
FROM <APP_DB>.INFORMATION_SCHEMA.TABLES WHERE IS_HYBRID = 'YES';
...
Instead of promoting `FG_<WORKLOAD>_DB`, **clone** the replicated `<APP_DB>` on DR into a throwaway database and validate against the clone. Production stays primary throughout.

```sql
-- ═══════════════════════════════════════════════════════════════════════════
-- §4 MODE B — CLONE-AND-TEST (Execute on the DR account, <DR_REGION>)
-- ═══════════════════════════════════════════════════════════════════════════
USE ROLE ACCOUNTADMIN;
...
CREATE DATABASE <APP_DB>_DRTEST CLONE <APP_DB>;
...
GRANT USAGE ON DATABASE <APP_DB>_DRTEST TO ROLE <TEST_RUNNER_ROLE>;
GRANT USAGE ON ALL SCHEMAS IN DATABASE <APP_DB>_DRTEST TO ROLE <TEST_RUNNER_ROLE>;
GRANT SELECT ON ALL TABLES IN DATABASE <APP_DB>_DRTEST TO ROLE <TEST_RUNNER_ROLE>;
GRANT SELECT ON FUTURE TABLES IN DATABASE <APP_DB>_DRTEST TO ROLE <TEST_RUNNER_ROLE>;

-- 5. Run the test plan against the clone (§9). Reads/writes on the clone don't affect the replica.

-- 6. Validate external-stage reachability (proves the DR cloud RBAC grant works)
LIST @<APP_DB>_DRTEST.<schema>.<external_stage>;
-- "access denied" => DR service identity is missing <STORAGE_RBAC_ROLE> (best-practices §4.1).

-- 7. Drop the test database when complete
DROP DATABASE <APP_DB>_DRTEST;

-- 8. Defensive verification — production should still be primary throughout
SHOW FAILOVER GROUPS;   -- FG_<WORKLOAD>_DB primary should still be <PROD_ACCOUNT>
```
...
> **Use this when** rehearsing (or responding to) a real regional outage. Promotes `FG_ACCOUNT_LEVEL` + `FG_<WORKLOAD>_DB` to `<DR_REGION>`. **`FG_NON_<WORKLOAD>_DBS` is intentionally NOT promoted.**

### 5.1 Failover steps

| Step | Description | Action | Est. time | Status |
|---|---|---|---|---|
| 5.1.1 | **Declare** | Authorize the `<WORKLOAD>` failover | 1–5 min | ☐ |
| 5.1.2 | Stop `<WORKLOAD>` writes on production (if reachable) | Pause ETL/pipes/apps | 1–2 min | ☐ |
...
| 5.1.5 | Promote workload DB group on DR | `ALTER FAILOVER GROUP FG_<WORKLOAD>_DB PRIMARY` | 1–2 min | ☐ |
| 5.1.6 | Promote the Connection on DR | `ALTER CONNECTION <WORKLOAD>_CONN PRIMARY` | < 1 min | ☐ |
| 5.1.7 | *(Private connectivity)* **Manual DNS flip** | Repoint **connection URL + OCSP URL** A records to `<DR_PE>` | 2–5 min | ☐ |
| 5.1.8 | Wait for DNS TTL | 60–300 s | 1–5 min | ☐ |
| 5.1.9 | Resume `<WORKLOAD>` tasks on DR | `ALTER TASK ... RESUME` (if not auto) | 1–2 min | ☐ |
...
| 5.1.12 | Confirm non-`<WORKLOAD>` DBs still read-write on production | `SHOW DATABASES` on production | 1–2 min | ☐ |
...
ALTER FAILOVER GROUP FG_<WORKLOAD>_DB PRIMARY;

-- 4. Promote the Connection so the connection URL resolves to DR
ALTER CONNECTION <WORKLOAD>_CONN PRIMARY;

-- 5. Verify promotion
SHOW FAILOVER GROUPS;   -- FG_ACCOUNT_LEVEL + FG_<WORKLOAD>_DB primary = <DR_ACCOUNT>
SHOW CONNECTIONS;       -- <WORKLOAD>_CONN is_primary = true on DR

-- 6. DO NOT promote FG_NON_<WORKLOAD>_DBS — those databases stay primary on production.

-- 7. Resume workload tasks if suspended on DR
SHOW TASKS IN DATABASE <APP_DB>;
-- ALTER TASK <APP_DB>.<schema>.<task> RESUME;   -- as needed

-- 8. Validate an external stage (confirms DR cloud RBAC)
-- LIST @<APP_DB>.<schema>.<external_stage>;
```

```text
-- ─────────────────────────────────────────────────────────────────────────────
-- §5.1.7  MANUAL DNS FLIP (<PRIVATE_CONNECTIVITY>) — network team
-- ─────────────────────────────────────────────────────────────────────────────
-- Repoint BOTH the connection URL and the OCSP URL from the production endpoint to DR:
--   <connection-url>.privatelink.snowflakecomputing.com  ->  <DR_PE>  (was <PRIMARY_PE>)
--   ocsp.<...>.privatelink.snowflakecomputing.com         ->  <DR_PE>
...
Once `<PRIMARY_REGION>` / the production account is healthy, fail back.
...
| 6.3 | Stop `<WORKLOAD>` writes on DR | Pause ETL/pipes/apps | 1–2 min | ☐ |
...
| 6.6 | Promote workload DB group on production | `ALTER FAILOVER GROUP FG_<WORKLOAD>_DB PRIMARY` | 1–2 min | ☐ |
| 6.7 | Promote Connection on production | `ALTER CONNECTION <WORKLOAD>_CONN PRIMARY` | < 1 min | ☐ |
| 6.8 | *(Private connectivity)* **Reverse the DNS flip** | Repoint connection URL + OCSP back to `<PRIMARY_PE>` | 2–5 min | ☐ |
| 6.9 | Wait for DNS TTL | 60–300 s | 1–5 min | ☐ |
| 6.10 | Resume `<WORKLOAD>` tasks on production | `ALTER TASK ... RESUME` | 1–2 min | ☐ |
| 6.11 | Validate connectivity via connection URL | Smoke test | 5–10 min | ☐ |
| 6.12 | Notify stakeholders | — | 1–2 min | ☐ |

```sql
-- ═══════════════════════════════════════════════════════════════════════════
-- §6 MODE A — FAILBACK (Execute on the production account, <PRIMARY_REGION>)
-- ═══════════════════════════════════════════════════════════════════════════
USE ROLE ACCOUNTADMIN;
ALTER FAILOVER GROUP FG_ACCOUNT_LEVEL REFRESH;
ALTER FAILOVER GROUP FG_<WORKLOAD>_DB REFRESH;
...
ALTER FAILOVER GROUP FG_<WORKLOAD>_DB PRIMARY;
ALTER CONNECTION <WORKLOAD>_CONN PRIMARY;
SHOW FAILOVER GROUPS;   -- primary = <PROD_ACCOUNT>
SHOW CONNECTIONS;       -- <WORKLOAD>_CONN is_primary = true on production
-- Then reverse the manual DNS flip (network team): connection URL + OCSP URL -> <PRIMARY_PE>
```
...
Run before the drill and again after; diff the outputs — they should be identical, proving the drill left production (and the non-`<WORKLOAD>` tenants) unchanged.

```sql
-- ═══════════════════════════════════════════════════════════════════════════
-- §7 BASELINE / POST-TEST SNAPSHOT — run on production (<PRIMARY_REGION>)
-- ═══════════════════════════════════════════════════════════════════════════
USE ROLE ACCOUNTADMIN;

-- 1. Workload object counts
SELECT TABLE_SCHEMA, COUNT(*) AS table_count, SUM(ROW_COUNT) AS rows, SUM(BYTES) AS bytes
FROM <APP_DB>.INFORMATION_SCHEMA.TABLES
...
-- 5. Non-workload databases must be unchanged and primary on production
SHOW DATABASES IN FAILOVER GROUP FG_NON_<WORKLOAD>_DBS;

-- 6. Non-replicable object counts (should be identical pre/post)
SHOW STREAMS IN DATABASE <APP_DB>;        -- check no new APPEND_ONLY
SHOW EXTERNAL TABLES IN DATABASE <APP_DB>;
...
Inbound-share databases **cannot be replicated**. For each share feeding `<APP_DB>`, resolve **before** the drill:

| Provider / Share | Workload dependency | Resolution | DR-side present? | Status |
|---|---|---|---|---|
| `<share_1>` | `<objects that join to it>` | Auto-Fulfillment / Dual-share | ☐ | ☐ |

**Options:** (1) **Auto-Fulfillment** — provider auto-fulfills into the DR region; (2) **Dual share** — provider shares to both `<PROD_ACCOUNT>` and `<DR_ACCOUNT>`.
...
|---|---|---|---|
| 9.1 | Data parity — row counts (**Tier 1 first**, §1.4.4) | `COUNT(*)` on Tier 1 tables vs production | Match (modulo continuous-ingest delta) |
...
| 9.4 | External stages | `LIST @stage` + read a file | Files reachable (cloud RBAC works) |
| 9.5 | Pipes / Snowpipe | Drop a test file in the DR container | Auto-ingest fires (`<PIPE_EVENT_PLUMBING>` configured) |
...
| 9.9 | Inbound shares | Query shared data on DR | Returns expected rows (§8) |
| 9.10 | Connection URL (Mode A) | Connect an app via `<CONNECTION_URL>` | Resolves to DR after DNS flip |
| 9.11 | OCSP / TLS (Mode A) | Establish TLS via private connectivity to DR | No cert validation failure |
...
| Cloud Network Team | Pre-stages endpoints/DNS; performs + times the manual DNS + OCSP flip |
| Cloud IAM Admin | Grants DR service identity `<STORAGE_RBAC_ROLE>` |
| `<WORKLOAD>` Data Engineering | Migrates pipelines to the connection URL; validates pipes/streams/tasks |
...
| # | Item | Detail |
|---|---|---|
| 11.1 | Private connectivity breaks auto-redirect | Every failover needs a **manual DNS change** — dominant RTO component. *(If applicable.)* |
...
| 11.4 | Append-only streams block refresh | Must be zero in `<APP_DB>` before the group is created. |
...
| 11.7 | DR service identity RBAC | Storage integration replicates, but the DR identity needs its own cloud RBAC grant. |
| 11.8 | Pipe event plumbing not replicated | `<PIPE_EVENT_PLUMBING>` must be built on DR. |
| 11.9 | Inbound shares not replicable | Auto-fulfillment or dual share required per provider (§8). |
| 11.10 | Don't promote `FG_NON_<WORKLOAD>_DBS` | Keeps non-`<WORKLOAD>` tenants read-write — the core design goal. |
...
| 2 | Finalize Connection object name | TBD | ☐ |
| 3 | Complete object audit; resolve append-only streams + dangling refs | TBD | ☐ |
| 4 | Resolve all inbound shares | TBD | ☐ |
| 5 | Migrate all `<WORKLOAD>` writers/readers to the connection URL | TBD | ☐ |
| 6 | Grant DR service identity cloud RBAC on shared storage | TBD | ☐ |
| 7 | Build DR-side pipe plumbing | TBD | ☐ |
| 8 | Pre-stage private endpoints + DNS (connection + OCSP); set TTL | Network team | ☐ |
| 9 | Run Mode B clone-and-test drill | TBD | ☐ |
| 10 | Schedule Mode A end-to-end drill (low-traffic window) | TBD | ☐ |

---

*Runbook template derived from the generic Snowflake BCDR best-practice set. Replace all `<TOKEN>` values. Living document — revise after each drill.*
